...
let isSameTeam(homeTeam: Team) (awayTeam: Team): bool =
    if homeTeam = awayTeam then true
    else false

let rootForTeam(team: Team): bool =
    match team with
    | { Coach = { Name = "Gregg Popovich" } } | { Coach = { FormerPlayer = true } } | { Name = "Chicago Bulls" } -> true
    | { Stats = { Wins = wins } } when wins >= 60 -> true
    | { Stats = { Wins = wins; Losses = losses } } when wins < losses -> true
    | _ -> false