...
let total books = 
    let rec bundle cart order =
        let size =
            ...
            |> List.filter (fun number ->
                number <= List.length cart)
            |> List.head
        let subtotal =
            computeDiscount (size) * (decimal size) * bookPrice
        let shrink =
            (cart
            |> List.take(size)
    ...
    let distinct =
        books
        |> List.groupBy id
        |> List.map (fun (_, instances) -> List.length instances)
        |> List.sortDescending
    
    [
        bundle distinct [5;4;3;2;1;0]
